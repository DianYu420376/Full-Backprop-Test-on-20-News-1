...
save_filename = 'semi_50_two_layer_pinv-ver1'
...
for epo in range(epoch):
    ...
    for (i, (inputs, label, L_batch)) in enumerate(dataloader):
       ...
       #train the lsqnonneg layers
        ...
        loss1 = criterion(inputs, torch.mm(S_lst[0], net.lsqnonneglst[0].A))
        loss2 = criterion(S_lst[0], torch.mm(S_lst[1], net.lsqnonneglst[1].A))
        loss3 = criterion(label_observed, torch.mm(S_observed, B))
        print(lambd1*loss1.data, lambd2*loss2.data, lambd3*loss3.data)
        loss = lambd1*loss1 + lambd2*loss2 + lambd3*loss3
        loss.backward()
